...
pub fn swap_internal<'b, 'info>(
    ...
    while state.amount_specified_remaining != 0 && state.sqrt_price_x64 != sqrt_price_limit_x64 {
        ...
        if state.sqrt_price_x64 == step.sqrt_price_next_x64 {
            ...
            if step.initialized {
                #[cfg(feature = "enable-log")]
                msg!("loading next tick1 {}", step.tick_next);
...
pub fn swap_internal_tmp<'b, 'info>(
    ...
    while state.amount_specified_remaining != 0 && state.sqrt_price_x64 != sqrt_price_limit_x64 {
        ...
        let swap_step = swap_math::compute_swap_step_tmp(
            step.sqrt_price_start_x64,
            target_price,
            state.liquidity,
            state.amount_specified_remaining,
        ...
        if state.sqrt_price_x64 == step.sqrt_price_next_x64 {
            ...
            if step.initialized {
                #[cfg(feature = "enable-log")]
                msg!("loading next tick2 {}", step.tick_next);

                let mut liquidity_net = next_initialized_tick.cross_tmp(
                    if zero_for_one {
                        state.fee_growth_global_x64
                    } else {
                        pool_state.fee_growth_global_0_x64
